...
//
        if (result==false) {
            if (this.index >= 0 && this.index < this.tokenList.size()) {
                TokenClass token = this.tokenList.get(this.index);

                if (token.error == null) {
                    String errorTk = token.valueP;
                    if (errorTk == null)
                        errorTk = token.classP;
        ...
        emptyAndReset(); // Assuming this method does what you intend it to do.
        return result;
    }
